...
names(sqanti_summary) <- c('bambu','rattle','rnabloom2','trinity')

num_per_chr <- lapply(sqanti_summary, function(x) {
  x %>% dplyr::count(chrom) %>% 
    dplyr::filter(str_detect(chrom, 'NC')) ## only canonical chromosome
...
num_ref <- read_gff('/home/users/allstaff/yan.a/lab_davidson/yan.a/ref/ncbi/pea_zw6/GCF_024323335.1_CAAS_Psat_ZW6_1.0_genomic.gtf') %>%
...
chimeric_summary_chr <- lapply(sqanti_summary, function(x){
  x %>% dplyr::filter(str_detect(isoform, '_dup')) %>% 
    dplyr::count(chrom) %>% 
    dplyr::filter(str_detect(chrom, 'NC'))
}) %>% purrr::reduce(full_join, by = 'chrom') %>%
  replace(is.na(.), 0)
...
sum_df <- lapply(sqanti_summary, function(x){
  anno.gene <- x %>% dplyr::count(associated_gene) %>%
    dplyr::mutate(category = ifelse(str_detect(associated_gene, '^LOC'), 'Annotated', ## potential fusion/read through, very few though
...
upset_sqanti(sqanti_summary, subcat = 'all', 
  ...
  upset(nsets = 4, sets = rev(names(sqanti_summary)), keep.order = T,
        order.by = 'freq', set_size.show = T, set_size.scale_max = 30000)
upset_sqanti(sqanti_summary, subcat = 'all', 
             names = names(sqanti_summary) , featuretype = 'transcript') %>%
  upset(nsets = 4, sets = rev(names(sqanti_summary)), keep.order = T,
        order.by = 'freq', set_size.show = T, set_size.scale_max = 40000)
dev.off()
...
requested_counts <- data.frame(sim_counts = rowSums(bambu_counts[,3:14]),
...
names(salmon_count) <- str_split(files, '/', simplify = T)[,5] %>%
  str_replace(., 'trinitystranded', 'trinity')
...
sqanti_summary_tmp <- sqanti_summary[c(2,2,2,3,3,4,4)]
...
quant_summary_cpm_recov <- lapply(quant_summary, function(x){
  ...
  separate(method, c(NA,NA,'assembler','quant'), remove = F) %>%
