...
lapply(filelist, function(x) {
  ...
  mutate(method = dirname(files)) %>%
  separate(method, c('assembler', 'clustering'), remove = F) %>%
  mutate(clustering = ifelse(clustering =='corset', 'corset', 'native')) %>%
  # dplyr::select(1:6) %>%
  pivot_longer(1:6) %>% 
...
df2 <- data.frame(quant_file = names(salmon_count)) %>%
  separate(quant_file, c(NA, NA, 'assembler', 'quant'), remove = F) %>%
...
sqanti_summary_tmp <- sqanti_summary[c(1,2,2,3,4,4)]
...
lapply(1:length(filelist), 
  ...
  stat_compare_means()
ggsave('plot/cluster_redundant_match_feature_sequin.pdf', width = 10, height = 10)
