...
busco_summary <- busco_summary %>% Reduce(rbind, .) %>%
  mutate(method = factor(method, levels = c('bambu','rattle','rnabloom2','isonform','trinitystranded'), 
...
run_summary <- data.frame(time_min=as.numeric(hms(c('00:56:34','00:35:08','19:56:23', '03:44:32', '05:18:38')), 'minutes') + c(0, 4*60*24,3*60*24,1*60*24,0),
                          memory_GB=c(39.08, 131.2, 217,198, 123.32),
                          tx_number=c(69478, 28356, 54954, 443874, 173041),
                          method = c('bambu','isonform','rattle', 'rnabloom2','trinity'))
