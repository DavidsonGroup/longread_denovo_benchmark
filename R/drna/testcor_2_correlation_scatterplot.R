...
all_quant <- readRDS("/vast/projects/lab_davidson/yan.a/dRNA_gencode/R/plot/all_quant.rds")
...
mat <- lapply(all_quant, function(x) {
  x$tx_exp_tiled$assemble_cpm.max
}) %>% Reduce(cbind, .) 
...
# 2. Loop to create 6 plots
highlight_idx <- which(names(truecpm_tx) == "ENST00000647112.1")
...
for (i in 1:6) {
  ...
  chunk_cor <- round(cor(chunk_df$rattle.sec, chunk_df$true_lcpm), 3)
  
  # Base plot
  p <- ggplot(chunk_df, aes(x = rattle.sec, y = true_lcpm)) +
    #geom_bin_2d() +
    geom_hex(bins = 50) +
    scale_fill_viridis_c(trans = "log10") +
    labs(
...
for (i in 1:6) {
  ...
  # Correlation for this chunk
  chunk_cor <- round(cor(chunk_df$rnabloom2.sec, chunk_df$true_lcpm), 3)
  
  # Base plot
  p <- ggplot(chunk_df, aes(x = rnabloom2.sec, y = true_lcpm)) +
    #geom_bin_2d() +
    geom_hex(bins = 50) +
    scale_fill_viridis_c(trans = "log10") +
    labs(
