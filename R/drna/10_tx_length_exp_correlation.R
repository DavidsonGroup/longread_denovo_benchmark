...
args <- c('../rnabloom2/', 
          '../rnabloom2/A549_MCF7_directRNA_corset/A549_MCF7_directRNA-clusters_mod.txt', 
          'rnabloom2_corset')
cor_rb <- get_exp_length_cor(args)

args <- c('../isonform/',
          '../isonform/A549_MCF7_directRNA_corset/A549_MCF7_directRNA-clusters_mod.txt',
          'isonform_corset')
cor_is <- get_exp_length_cor(args)

args <- c('../trinitystranded/',
          '../trinitystranded/A549_MCF7_Illimina_corset/A549_MCF7_Illimina-clusters_mod.txt',
          'trinity_corset')
cor_tr <- get_exp_length_cor(args)

args <- c('../rattle/', 
          '../rattle/A549_MCF7_directRNA/fx2tab.txt', 
          'rattle_rattle')
...
rt_gene <- get_gene_tx(args[2],
                       list.files(dirs[grepl('sqanti3$', dirs)], 
                                  pattern = 'classification.txt', full.names = T, recursive = T),
                       method = method)
rownames(rt_gene) <- rt_gene$isoform
# 
cor_rt2 <- rt_gene %>%
...
bambu_counts <- readRDS('../bambu/A549_MCF7_directRNA_dge/default/se1.rds')
...
rbind(data.frame(method = 'rattle_count', cor = cor_rt2),
