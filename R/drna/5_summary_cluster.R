...
df2 <- data.frame(quant_file = names(salmon_count)) %>%
  separate(quant_file, c(NA, NA,NA, NA, 'assembler', 'quant'), remove = F) %>%
  mutate(assembler = ifelse(str_detect(assembler, 'trinity'), 'trinity', assembler)) %>%
...
cor2 <- left_join(cor2 %>% 
                    unite(dataset, 'assembler', remove = F) %>%
                    filter(quant %in% c('onts', 'map')) %>%
                    dplyr::select(c(2,3,4,9)),
                  cor2 %>%   
                    unite(dataset, 'assembler', remove = F) %>%
                    filter(quant %in% c('ontp', 'align')) %>% 
                    dplyr::select(c(2,3,4,9)), by = 'dataset',
                  suffix = c('.sec', '.pri')) 
