...
gid <- c('R2_68', 'R2_7','R1_23',
         'ENSG00000106546.14','ENSG00000215717.7','ENSG00000149806.11')

rb_tx1 %>% left_join(tx2gene, by = c('txid' = 'transcript_id')) %>% 
  select(counts.max, barcode01_10m_sorted, txid, gene_id) %>%
  left_join(rt_tx1 %>% left_join(tx2gene, by = c('txid' = 'transcript_id')) %>% 
              select(counts.max, barcode01_10m_sorted, txid, gene_id),
            by = c('txid','gene_id','barcode01_10m_sorted'), 
  ...
  pivot_longer(cols = c(counts.max.rnabloom2, counts.max.rattle, barcode01_10m_sorted),
               values_to = 'counts',
               names_to = 'source') %>%
  mutate(source = factor(source, levels = c("counts.max.rnabloom2", "counts.max.rattle" ,"barcode01_10m_sorted"))) %>%
  ggplot(aes(x = txid, y = counts, colour = source)) +
  geom_point(position = position_jitter(width = 0.2, height = 0)) +
  facet_wrap( . ~ gene_id, scales = 'free', nrow = 2) +  # Rows by group, columns by geneid
...
data.frame(total_counts_rb = count_s1_rb$counts %>% sum,
           missedtx_counts_rb = rb_tx1 %>% filter(isassemble=='not assembled') %>% pull(barcode01_10m_sorted) %>% sum,
           total_counts_rt = count_s1_rt$counts %>% sum,
           missedtx_counts_rt = rt_tx1 %>% filter(isassemble=='not assembled') %>% pull(barcode01_10m_sorted) %>% sum 
) %>% pivot_longer(1:4) %>%
