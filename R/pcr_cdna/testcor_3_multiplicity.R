...
rt_tx <- quant_summary$merged_10m_quant_rattle_onts$lcpm_tx
...
rb_ref <- sqanti_summary$rnabloom2_10m %>% 
  filter(associated_transcript != 'novel') %>%
  group_by(associated_transcript) %>%
  summarise(n = n())

rb_tx <- quant_summary$merged_10m_quant_rnabloom2_onts$lcpm_tx
