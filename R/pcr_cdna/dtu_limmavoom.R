...
data <- ifelse(data[1] =='trinity' & data[3] == '10m', 'ilu', 
               ifelse(data[1] %in% c('rnaspades','rnabloom2hybrid'), 'hybrid_merged', data[3])) # for short read name
...
contr <- makeContrasts(H1975 - HCC827, levels = design)

pdf(paste0(outputdir, '/dte_mvplot.pdf'), width = 5, height = 5)
...
pdf(paste0(outputdir, '/dte_mdplot.pdf'), width = 5, height = 5)
