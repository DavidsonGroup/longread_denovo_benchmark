...
truecpm_gene <- log2(gene_counts_bambu$counts + 1)[geneorder, ] %>% 
  as.matrix() %>%
  c()
...
cor_noseq_bambu <- lapply(bambu_dir, function(x){
  gene <- readRDS(paste0(x, '/x.rds'))
  gene <- log2(gene$counts + 1)[geneorder, ] %>% 
    as.matrix() %>% c()
  cor.gene <- cor(gene, truecpm_gene)
  
  tx <- readRDS(paste0(x, '/y.rds'))
  tx <- log2(tx$counts + 1)[txorder, ] %>% 
    as.matrix() %>% c()
  cor.tx <- cor(tx, truecpm_tx)  
  res <- data.frame(cor.tx = cor.tx, cor.gene = cor.gene)
}) %>%
  rbindlist() %>%
  mutate(data = c('bambu.sec'))

cor_noseq <- rbind(cor_noseq_bambu, cor_noseq)
