...
args <- c('../simulation_1m/rnabloom2/', 
          '../simulation_1m/rnabloom2/ONT_merged_corset/ONT_merged-clusters_mod.txt', 
          'rnabloom2_corset')
cor_rb <- get_exp_length_cor(args)

args <- c('../simulation_1m/isonform/', 
          '../simulation_1m/isonform/ONT_merged_corset/ONT_merged-clusters_mod.txt', 
          'rnabloom2_corset')
cor_is <- get_exp_length_cor(args)

args <- c('../simulation_1m/trinity/', 
          '../simulation_1m/trinity/Ilu_merged_corset/Ilu_merged-clusters_mod.txt', 
          'trinity_corset')
cor_tr <- get_exp_length_cor(args)

args <- c('../simulation_1m/rattle/', 
          '../simulation_1m/rattle/fx2tab.txt', 
          'rattle_rattle')
cor_rt2 <- get_exp_length_cor(args)
...
rbind(data.frame(method = 'rattle_count', cor = cor_rt),
      data.frame(method = 'rattle_corset', cor = cor_rt2),
