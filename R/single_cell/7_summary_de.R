...
true_dge <- readRDS("bambu_bambu_oarfish/dge_gene.rds") %>% filter(adj.P.Val < 0.05) %>% 
  left_join(bambu_anno$asm_map, 
            by = c('gene' = 'geneid')) %>% 
  pull(assigned_gene) %>% na.omit()
  
true_dte <- readRDS("bambu_bambu_oarfish/dte_tx.rds") %>% filter(adj.P.Val < 0.05) %>% pull(isoform) %>% na.omit()

true_dtugene <- readRDS("bambu_bambu_oarfish/dtu_gene.rds") %>% filter(FDR < 0.05) %>%
  left_join(bambu_anno$asm_map, by = c('geneid' = 'geneid')) %>% 
  pull(assigned_gene) %>% na.omit()

true_dtutx <- readRDS("bambu_bambu_oarfish/dtu_tx.rds") %>% filter(FDR < 0.05) %>% pull(isoform) %>% na.omit()
  ...
  if (x %in% (1:ntest)) {
    test4[[x]] %>% 
      ...
      left_join(filelist[[x]]$asm_map %>%
                  mutate(new_gid = str_replace_all(geneid, '_', '-')), # seurat does this conversion, so need to change back
                by= c('rowname' = 'new_gid')) %>% 
      pull(assigned_gene)
  } else { # for bambu
    test4[[x]] %>% 
...
df_dtutx_all %>%
  ...
  filter(quant %in% c('oarfish'),
         (assembler == "rnabloom2" & clustering == "corset") |
           (assembler != "rnabloom2" & clustering != "corset")) %>%
