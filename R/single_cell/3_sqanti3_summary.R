...
bambu_counts <- read.table('../bambu/PB_merged_dge/default/counts_transcript.txt', header = T, sep = '\t') 
requested_counts <- data.frame(sim_counts = rowSums(bambu_counts[,3:5]),
                               transcript_id = bambu_counts$TXNAME, 
                               gene_id = bambu_counts$GENEID) %>%
  mutate(length = 1, requested_counts = 1)

true_dte <- readRDS('bambu_bambu_oarfish/dte_tx.rds') %>% filter(adj.P.Val < 0.05) %>% pull(isoform)
true_dtutx <- readRDS('bambu_bambu_oarfish/dtu_tx.rds') %>% filter(FDR < 0.05) %>% pull(isoform)
...
sqanti_summary_tmp <- sqanti_summary[c(2,2,3,3,4,4)]

quant_summary <- list() 

# for ONT and Illumina
quant_summary[1:6] <- lapply(1:6, function(x){
...
quant_summary_cpm_recov <- lapply(quant_summary, function(x){
  ...
  mutate(assembler = factor(assembler, levels = c('bambu','rattle','rnabloom2','isonform')))
...
lcpm.cor <- data.frame(gene_cor = sapply(quant_summary, function(x) cor(x$lcpm_gene[,1:2] )[1,2], simplify = T), 
  ...
  separate(method, c('col1','col2','salmon','assembler','quant'), remove = F) %>%
  mutate(assembler = factor(assembler, labels = c('bambu','rattle','rnabloom2','isonform'), 
