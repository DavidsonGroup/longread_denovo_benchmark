...
names(cols) <- c('ref','bambu','isonform','rattle','rnabloom2')
shapes <- c(15,15,16,17,17,17)
names(shapes) <- c('ref','bambu','corset','isonclust','rattle')
...
cor1 <- left_join(cor.corset, cor.native,
...
cor2 <- left_join(cor2 %>% 
                    unite(dataset, 'assembler', remove = F) %>%
                    filter(quant %in% c('onts', 'map')) %>%
                    dplyr::select(c(2,3,4,8)),
                  cor2 %>%   
                    unite(dataset, 'assembler', remove = F) %>%
                    filter(quant %in% c('ontp', 'align')) %>% 
                    dplyr::select(c(2,3,4,8)), by = 'dataset',
                  suffix = c('.sec', '.pri')) 
...
cor.final %>%
  dplyr::select(c(2,3,5:8, 11:16,18:19)) %>% # 12 columns of correlation
  pivot_longer(-c(1:2)) %>% 
...
lapply(names(filelist), function(x) {
  ...
  subset_known <- filelist[[x]]$sqanti_anno %>%
    ...
    dplyr::select(associated_transcript, associated_gene)
...
sqanti_summary_tmp <- sqanti_summary[c(1,2,2,3,3,4)]
